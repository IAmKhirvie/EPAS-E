...
class ClassController extends Controller
{
    ...
    public function index(Request $request): View
    {
        ...
        // Get instructor's accessible sections
        $instructorSections = $this->getInstructorAccessibleSections($viewer);

        // INSTRUCTOR RESTRICTION: Only show their assigned sections
        if ($viewer->role === Roles::INSTRUCTOR) {
            ...
            if ($sectionFilter && !$instructorSections->contains($sectionFilter)) {
                $sectionFilter = null; // Show all their sections
            }
        }
        ...
        // Get all advisers for each section (supports multiple per section)
        ...
        $studentsBySection = $studentsQuery
            ->orderBy('section')
            ->orderBy('last_name')
            ->orderBy('first_name')
            ->get()
        ...
        // If section filter is applied, show table view
        if ($sectionFilter) {
            return $this->showSectionDetail(
                $sectionFilter, $search, $allSections,
                $studentsBySection, $instructors, $advisersBySection, $viewer
            );
        }
